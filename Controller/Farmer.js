...
exports.verifyOtpAndLogin = async (req, res) => {
    try {
        ...
        // Check if account is marked for deletion (pending or already deleted)
        if (farmer && (farmer.deletionStatus === "pending_deletion" || farmer.deletionStatus === "deleted")) {
            return res.status(403).json({
                success: false,
                message: "Your account has been marked for deletion and is no longer active. Please contact support if this is a mistake.",
                code: "ACCOUNT_PENDING_DELETION"
    ...
    try {
        ...
        try {
            if (farmer.fcmToken) {
                ...
                await sendPushNotification(farmer.fcmToken, title, body, { notificationType: "ACCOUNT_STATUS_UPDATED", status: farmer.status });
            }
        } catch (pushErr) {
            console.error("Push notification failed:", pushErr.message);
        }
